...
-- From Games, list the top 10 most expensive games (cost in descending order).

SELECT TOP 10 title, cost FROM Games ORDER BY cost DESC

-- From Users, find all users whose wallet is greater than 500 and who live in 'USA'.

SELECT username, wallet, country FROM Users WHERE wallet>500 AND country='USA'

-- From Payment, display payments where vacation_days between 10 and 25 or work_hours less than 35.

SELECT payment_id, vacation_days, work_hours FROM Payment WHERE (vacation_days BETWEEN 10 AND 25) OR (work_hours<35)

-- From Purchasement, list entries where bank like 'B%' and sales_percentage not between 10 and 50.

SELECT bank, sales_percentage FROM Purchasement WHERE bank LIKE 'B%' AND sales_percentage NOT BETWEEN 10 AND 50

-- From Systems, show all records where device_type in ('PC', 'Console') and ram >= 16.

SELECT device_type, ram FROM Systems WHERE device_type IN ('PC', 'Console') AND ram >= 16

-- From Roles, retrieve all where responsibilities like '%management%' or privileges is null.

SELECT role_title, responsibilities, privileges FROM Roles WHERE responsibilities LIKE '%management%' OR privileges IS NULL

-- From Creator, find creators who have one_many_people = 1 (team creators) and projects is not null.

SELECT creator_id, one_many_people, projects FROM Creator WHERE one_many_people = 1 AND projects IS NOT NULL

-- From Inventory, return records where played_hrs > 1000 and achievements like '%gold%'.

SELECT inventory_id, played_hrs, achievements FROM Inventory WHERE played_hrs > 1000 AND achievements LIKE '%gold%'

-- From Users, display users born between `'1995-01-01'` and `'2005-12-31'` and whose email ends with '@gmail.com'.

SELECT username, email, birth_date FROM Users WHERE birth_date BETWEEN '1995-01-01' AND '2005-12-31' AND email LIKE '%@gmail.com'

-- From Games, show games where genre in ('Action', 'RPG', 'Strategy') and dimension like '3D%'.

SELECT title, genre, dimension FROM Games WHERE genre IN ('Action', 'RPG', 'Strategy') AND dimension LIKE '3D%'
